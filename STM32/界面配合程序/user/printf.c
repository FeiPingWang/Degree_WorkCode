...
void Usart1_Send_String(char *buf, unsigned short n)
{
    unsigned short i;
    for(i = 0; i < n; i ++){
    while((USART1->SR & USART_FLAG_TXE) == RESET);	//判断串口接收数据是否完成，如果没有完成则继续等待，不知道这个地方是否需要将标志清掉
    USART1->DR=buf[i];        						//将buf里面的数据从串口发送
    }

}

void UART1_Print(char *format,...)					//任何形式的数据都可以发送
{
    char buf[200];
    va_list ap;
    unsigned short n;
...
void UART2_Print(char *format,...)					//任何形式的数据都可以发送
{
    char buf[200];
    va_list ap;
    unsigned short n;
